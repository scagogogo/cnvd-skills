...
加速乐逆向已剥离为独立模块 [**go-jsl**](./gojsl)（`github.com/scagogogo/go-jsl`，package `jsl`），可脱离 CNVD 业务直接访问任意被加速乐保护的站点，作为通用加速乐绕过库独立 `go get`：
...
本库通过 go.mod `require` + 本地 `replace ./gojsl` 依赖该模块。`CnvdSkills` 持有一个默认 `jsl.JslClient` 实例（直连、不限时、不配识别器），可通过 `JslClient()` 获取；带 `Config` 的请求在 `requestWithRetry` 内按请求派生独立客户端，并发安全。验证码识别器配置类型为 `jsl.CaptchaSolver`，内置实现见 go-jsl 包。
